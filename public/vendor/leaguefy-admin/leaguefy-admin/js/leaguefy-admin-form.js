...
leaguefy.form = {
  ...
  addAjaxSubmit: function (container = null) {
    // forms that should be submitted with ajax
    container = container ? `${container} form` : 'form';
    Array.from(document.querySelectorAll(container)).forEach((form) => {
      if (
        form.getAttribute('pjax-container') != null &&
        !form.classList.contains('has-ajax-handler')
      ) {
        form.addEventListener('submit', function (event) {
          leaguefy.form.submit(event.target);
  ...
  submit: function (form, result_function) {
    ...
    if (leaguefy.form.validate(form)) {
      if (method === 'post' || method === 'put') {
        ...
        if (typeof result_function !== 'function') {
          leaguefy.ajax.setUrl(url);
        }
      }

      if (typeof result_function === 'function') {
        leaguefy.ajax.request(url, obj, result_function);
      } else {
        leaguefy.ajax.load(url, obj);
      }
    } else {
  ...
  footer: function () {
    document.querySelectorAll('.after-submit').forEach((check) => {
      check.addEventListener('click', function () {
        document
          .querySelectorAll(".after-submit:not([value='" + this.value + "']")
          .forEach((other) => {
            other.checked = false;
          });
      });
  ...
  validate: function (form) {
    ...
    if (form.classList.contains('needs-validation')) {
      ...
      leaguefy.form.check_tab_errors();
    }
    return res;
  },
};
